...
// Elimina los pasajes existentes
$sql_borrar_pasajes = "DELETE FROM Pasaje";
$conn->query($sql_borrar_pasajes);

// Reinicia el contador de AUTO_INCREMENT para que los IDs comiencen desde 1
$sql_reset_autoincrement = "ALTER TABLE Pasaje AUTO_INCREMENT = 1";
$conn->query($sql_reset_autoincrement);

// Verificar viajes disponibles entre julio y diciembre
$sql_viajes = "SELECT v.Id_Viaje, v.Fecha_salida, v.Hora_salida, v.Id_Bus, vd1.Precio AS Precio_1_Piso, vd2.Precio AS Precio_2_Piso
               ...
               WHERE v.Fecha_salida BETWEEN '2025-07-01' AND '2025-12-31'";
...
if (count($viajes) == 0) {
    die("No hay viajes disponibles entre julio y diciembre para asignar.");
}
...
foreach ($clientes as $dni_cliente) {
    ...
    foreach ((array)$viajes_asignados as $index) {
        ...
        // Generar fecha de compra aleatoria entre 1 y 16 de julio
        $fecha_compra = date('Y-m-d', strtotime("2025-07-" . rand(1, 16)));
        ...
        $stmt->bind_param("sssii", $fecha_compra, $codigoQR, $id_viaje, $dni_cliente, $id_asiento);
        $stmt->execute();
...
echo "Pasajes regenerados correctamente con IDs reiniciados y fechas de compra entre el 1 y el 16 de julio.";
